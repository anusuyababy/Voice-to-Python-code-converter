...
def generate_code(query, df):
    if operator.contains(query, "import library"):
        ...
        d = df.corr().to_html()
        return d
    elif operator.contains(query, "correlations"):
        d = df.corr().to_html()
        return d
        ...
        f = q.to_html()
        return f
    elif operator.contains(query, "column labels"):
        s = df.columns
        d = s.to_frame()
        q = d.reset_index(drop=True)
        f = q.to_html()
        return f
    elif operator.contains(query, "column"):
        s = df.columns
        d = s.to_frame()
        q = d.reset_index(drop=True)
        f = q.to_html()
        return f
    elif operator.contains(query, "columns"):
        s = df.columns
        d = s.to_frame()
        q = d.reset_index(drop=True)
        f = q.to_html()
        ...
        s = df.head().to_html()
        return s
    elif operator.contains(query, "fill in"):
        df.replace('?', np.NaN, inplace=True)
        # seperating categorical feature
        feat_cat=[feature for feature in df.columns if df[feature].isnull().sum()>0 and df[feature].dtype =='O']
        # seperating numerical feature
        ...
        z = report.to_html()
        return z
    elif operator.contains(query, "CART"):
        le = LabelEncoder()
        objList = df.select_dtypes(include = "object").columns
        print (objList)
        for feat in objList:
